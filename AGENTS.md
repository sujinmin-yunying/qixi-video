...
- `APP_ENV` 是环境最高优先级真相源，只接受 `local`、`test`、`prod`；Vercel 部署上下文按 development/preview/production 映射为三态，其他平台必须显式设置。
...
- 2026-08-06：手机电影票取消横向偏移；预览与固定 660×488 导出票面解耦。
- 2026-08-06：手机票据信息改为两列键值布局，运行环境徽标退出固定遮挡层。
- 2026-08-06：Vercel 部署上下文映射为 local/test/prod，修复生产函数因缺少 APP_ENV 无法启动。
